...
void DlgHisto::InitDlg()
{
	...
	m_r.EnableTarget(0);
	m_r.SetIcon(IDI_HISTOGRAM_R,BS_LEFT);
	m_r.SetCheck(1);
	m_r.EnableWindow(1);
	...
	m_g.EnableTarget(0);
	m_g.SetIcon(IDI_HISTOGRAM_G,BS_LEFT);
	m_g.SetCheck(1);
	m_g.EnableWindow(1);
	...
	m_b.EnableTarget(0);
	m_b.SetIcon(IDI_HISTOGRAM_B,BS_LEFT);
	m_b.SetCheck(1);
	m_b.EnableWindow(1);
	...
	m_gray.SetIcon(IDI_HISTOGRAM_GRAY,BS_LEFT);
	m_gray.SetCheck(1);
	m_gray.EnableWindow(1);

	m_ren=m_gen=m_ben=m_grayen=1;

	CButton* plog = (CButton*)GetDlgItem(IDC_HISTOGRAM_LOG);
...
void DlgHisto::OnPaint() 
{
	...
	CImageProcessingDoc* pDoc = (CImageProcessingDoc*) ((CMainFrame*)AfxGetMainWnd())->GetActiveFrame()->GetActiveDocument();
	...
	if (pDoc && pDoc->GetImage()) {

		if (pDoc->GetHistogramMax()){
			int ybase = r.bottom-10;
			if (m_logen){
				float yratio = (r.bottom - r.top - 50)/(float)log10((float)(pDoc->GetHistogramMax()+1));
				if (m_ren){
					CPen redPen(PS_SOLID, 1, RGB(222, 0, 0));
					pOldPen = dc.SelectObject(&redPen);
					dc.MoveTo(10, (int)(ybase-log10((float)(1+pDoc->GetHistogramRed(0)))*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-log10((float)(1+pDoc->GetHistogramRed(x)))*yratio));
				}
				if (m_gen){
					CPen greenPen(PS_SOLID, 1, RGB(0, 222, 0));
					pOldPen = dc.SelectObject(&greenPen);
					dc.MoveTo(10, (int)(ybase-log10((float)(1+pDoc->GetHistogramGreen(0)))*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-log10((float)(1+pDoc->GetHistogramGreen(x)))*yratio));
				}
				if (m_ben){
					CPen bluePen(PS_SOLID, 1, RGB(0, 0, 222));
					pOldPen = dc.SelectObject(&bluePen);
					dc.MoveTo(10, (int)(ybase-log10((float)(1+pDoc->GetHistogramBlue(0)))*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-log10((float)(1+pDoc->GetHistogramBlue(x)))*yratio));
				}
				if (m_grayen){
					CPen grayPen(PS_SOLID, 1, RGB(64, 64, 64));
					pOldPen = dc.SelectObject(&grayPen);
					dc.MoveTo(10, (int)(ybase-log10((float)(1+pDoc->GetHistogramGray(0)))*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-log10((float)(1+pDoc->GetHistogramGray(x)))*yratio));
				}
				dc.SelectObject(pOldPen);
			} else {
				float yratio = (r.bottom - r.top - 50)/(float)pDoc->GetHistogramMax();
				if (m_ren){
					CPen redPen(PS_SOLID, 1, RGB(222, 0, 0));
					pOldPen = dc.SelectObject(&redPen);
					dc.MoveTo(10, (int)(ybase-pDoc->GetHistogramRed(0)*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-pDoc->GetHistogramRed(x)*yratio));
				}
				if (m_gen){
					CPen greenPen(PS_SOLID, 1, RGB(0, 222, 0));
					pOldPen = dc.SelectObject(&greenPen);
					dc.MoveTo(10, (int)(ybase-pDoc->GetHistogramGreen(0)*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-pDoc->GetHistogramGreen(x)*yratio));
				}
				if (m_ben){
					CPen bluePen(PS_SOLID, 1, RGB(0, 0, 222));
					pOldPen = dc.SelectObject(&bluePen);
					dc.MoveTo(10, (int)(ybase-pDoc->GetHistogramBlue(0)*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-pDoc->GetHistogramBlue(x)*yratio));
				}
				if (m_grayen){
					CPen grayPen(PS_SOLID, 1, RGB(64, 64, 64));
					pOldPen = dc.SelectObject(&grayPen);
					dc.MoveTo(10, (int)(ybase-pDoc->GetHistogramGray(0)*yratio));
					for (int x=1; x<256; x++)
						dc.LineTo(x+10, (int)(ybase-pDoc->GetHistogramGray(x)*yratio));
				}
				dc.SelectObject(pOldPen);
			}
		}
		...
		{
			pDoc->CalculateHistogram();
			if (pDoc->GetHistogramMax()) Invalidate();
		}
	}
}
